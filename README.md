...
Built by Team C | Full-Stack Project · React + Spring Boot + MySQL + Gemini AI Working under the guidance of Shakthi Gopalakrishnan
...
springdoc-openapi (planned — see Roadmap)
AI Integration
...
Student clicks Generate Roadmap in the React UI.
Axios sends a JWT-authenticated POST to the backend.
Spring Security validates the token and resolves the authenticated user.
The roadmap service layer pulls the student's skill-gap data via repository, builds a prompt, and calls the Gemini API.
Gemini's response is parsed, persisted (so the roadmap survives reloads), and returned to the client as a DTO.
The UI renders the roadmap; progress against it is tracked on subsequent visits.
📂 Project Structure
...
Edit application.properties with your local MySQL credentials, Google OAuth2 keys, JWT secret, mail credentials, and Gemini API key (see Environment Variables).
...
📌 Next step: wire up springdoc-openapi so this table is generated and always accurate instead of hand-maintained (see Roadmap).
...
Review & merge — at least one teammate reviews before merging into main.
Keep main deployable at all times — avoid pushing directly to it once the team grows past solo work.
...
🤝 Contributing
Pick up an issue or feature from the team backlog.
Follow the Git Workflow above — feature branch, clear commits, PR into main.
Keep entities (model/) out of API responses — always return through dto/.
Never commit real secrets — only .example templates belong in version control.
Add/update tests for any new service or controller logic where practical.
📄 License
